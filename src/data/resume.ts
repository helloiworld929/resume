  ...
  projects: [
    {
      ...
      proofPoints: [
        ...
        { value: 'players 列表', label: '替代固定头像与性别字段' },
      ...
      responsibilities: [
        '独立完成认证、场地查询、预约、取消、核销和管理端等核心链路，熟悉从页面到 Controller、Service、Mapper 和数据库的完整调用过程。',
        '使用数据库悲观锁串行化同一场地的容量检查，通过带原状态条件的更新保证取消、核销和爽约流转的一致性。',
        '将场地接口中固定的头像、性别槽位重构为 players 列表，降低字段冗余并提升人数规则的扩展性。',
        '通过 @Scheduled 每 5 分钟扫描开始时间已过 30 分钟仍未核销的预约并自动流转为爽约，条件更新兜底幂等，重复扫描或与取消并发时仅流转一次。',
      ],
      deployment: {
    ...
    },
    {
      ...
      overview: '项目用于练习并验证 RabbitMQ 异步消息链路。创建批量任务后，系统为每位接收人生成独立发送记录，并在数据库事务提交后发布 recordId；消费者读取数据库中的权威状态，完成邮件发送、状态更新和任务进度聚合。项目通过 Docker Compose 完成 App、MySQL、RabbitMQ 与 MailHog 的本地交付和端到端验收。',
      responsibilities: [
        '设计“通知任务—发送记录”模型，将批量接收人拆分为独立消息，使单个地址的失败不会阻塞整批任务，并持久化成功数、失败数和待处理数。',
        '使用手动 ACK 控制消息确认时机；重试或死信消息通过 Publisher Confirm 与 Return 检查接收、路由结果后，再确认原消息，发布失败时关闭 Channel 触发恢复。',
        '通过 TTL 重试队列将失败消息延迟后重新路由到消费队列，达到最多 3 次尝试后写入最终失败状态并进入死信队列。',
        '消息只携带 recordId，以数据库记录作为状态权威来源；对 SUCCESS 记录进行幂等短路，并支持最终失败记录手动重发。',
      ],
      deployment: {
        method: 'Docker Compose',
